...
public class Reloj extends Thread // Hilo de los relojes
{
    ...
    public void run() {
        while (true) // El reloj continuara hasta ser detenido
        {
            // Avanzamos el tiempo
            if (!detenido) {
                seg++; // Solo avanza si no esta detenido el reloj
            }
            if (seg >= 60) {
                min++;
                seg = 0;
    ...
    public void go_Slow() {
        time += 100;
    } // Hace que el reloj avance 1 segundo cada 2...3...4 etc 

    public void go_Fast() {
        time = (time - 100 <= 0) ? 100 : time - 100;
    } // No queremos que el reloj vaya 'infinito' rapido but gottagofast 
    ...
    // Bloque set
    public void set_Hora(double newHora) {
        hora = newHora;
    }
    ...
    public void set_Seg(double newSeg) {
        seg = newSeg;
    }
